...
This document describes the VRA gateway network's automated image generation system for blog posts. Because this is the master template repo, **new site repos created from this template automatically inherit this file at `.claude/IMAGE-GENERATION.md`** - so any new site gets the image-generation docs for free.
...
## Per-site setup requirements (for repos created from this template)
...
7. **Repo variable `IMAGE_GEN_ENABLED` = `"true"`** - must be set for each new site
8. Repo variable `R2_PUBLIC_BASE` - e.g. `https://pub-c7a09e1ddb7c45e6a38fcdca1e4b6897.r2.dev`
9. `SITE_ID` in workflow YAML set to the R2 folder prefix for the new site
